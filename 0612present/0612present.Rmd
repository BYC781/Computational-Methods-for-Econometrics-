...
Define the data $(\mathbf{x}_i, y_i)$, $i = 1,2,...,N$
where $x_{ij}$ are standardized, s.t. $\frac{1}{N}\sum_i x_{ij} = 0$, $\frac{1}{N}\sum_i x_{ij}^2 = 1$
...
$X$ is orthogonal, i.e., $X^T X = I$, then OLS solution be $\hat{\boldsymbol\beta}^o = X^T Y$.
\begin{align*}
& \min_{\boldsymbol{\beta}} \quad -Y^T X\boldsymbol{\beta} + \frac{1}{2}\boldsymbol{\beta}^T X^T X \boldsymbol{\beta} + \gamma ||\boldsymbol{\beta}|| \\
...
-   Then we estimate the standard error of $\tilde{\boldsymbol{\beta}}$ as follows:

$$
SE(\tilde{\boldsymbol{\beta}}) = \sqrt{\frac{1}{B} \sum_{b=1}^{B} \left(\tilde{\beta}^{(b)} - \frac{1}{B} \sum_{r=1}^{B} \tilde{\beta}^{(r)}\right) \left(\tilde{\beta}^{(b)} - \frac{1}{B} \sum_{r=1}^{B} \tilde{\beta}^{(r)}\right)^T}
$$
...
Hence, at the lasso estimate, we may approximate the solution by a ridge regression of the form $\boldsymbol{\beta}^* = (\boldsymbol{X}^T\boldsymbol{X} + \lambda \boldsymbol{W}^{-} )^{-1} \boldsymbol{X}^T \boldsymbol{Y}$.  


where $\boldsymbol{W} = \text{diag}(|\hat\beta_j^{lasso}|)$, and $\boldsymbol{W}^{-}$ denotes the generalized inverse of $\boldsymbol{W}$.
The covariance matrix of the estimates may then be approximated by

\begin{equation}
(\boldsymbol{X}^T\boldsymbol{X} + \lambda \boldsymbol{W}^{-})^{-1} \boldsymbol{X}^T\boldsymbol{X} (\boldsymbol{X}^T\boldsymbol{X} + \lambda \boldsymbol{W}^{-})^{-1} \hat\sigma^2
\end{equation}
...
## Stein's Unbiased Risk Estimation (SURE)
$$
R(\hat{\boldsymbol{\beta}}(\gamma)) \approx \hat{\tau}^2 \left\{p - 2 \# \left(j: \left|\frac{\hat{\beta}_j^o}{\hat{\tau}}\right| < \gamma\right) + \sum_{j=1}^p \max \left(\left|\frac{\hat{\beta}_j^o}{\hat{\tau}}\right|, \gamma\right)^2\right\}
$$
...
We fix $t \geq 0$, the problem (12) can be expressed as a least squares problem with $2^p$ inequality constraints, corresponding to the $2^p$ different possible signs for the $\beta_j$s.
\begin{align}
(\hat{\alpha}, \hat{\boldsymbol{\beta}}) &:= \arg\min \left\{ \sum_{i=1}^N (y_i - \alpha - \sum_j \beta_j x_{ij})^2 \right\} \text{ s.t. } \sum_j|\beta_j| \leq t
\end{align}
Then the condition $\sum |\beta_j| \leq t$ is equivalent to $\boldsymbol{\delta}_i^T \boldsymbol{\beta} \leq t$ for all $i$.
where $\boldsymbol{\delta}_i = (\pm1, \pm1, ..., \pm1) , i =1, 2,..., 2^p$ be the $p$-tuples.
...
where $G_E$ is the matrix whose rows are $\delta_i$ for $i \in E$ and  
$\boldsymbol{1}$ is a vector of 1s of length equal to the number of rows of $G_E$.
