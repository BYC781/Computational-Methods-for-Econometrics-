...
for (t in 2:T){
    ...
    while (accept == 0){
        if (t<2){
            ...
            lambda_prime <- rnorm(1, lambda_T[t-1], 0) * 0.95 + rnorm(1, lambda_T[t-1], 0.1^2) * 0.05
        }else{
            lambda_prime <- rnorm(1, lambda_T[t-1], 2.38^2 * var(lambda_T[1:t-1])) * 0.95 + rnorm(1, lambda_T[t-1], 0.1^2) * 0.05
        }
...
posterior_result$beta_mean <- sapply(1:26, function(k){mean(beta_T[,k])})
posterior_result$beta_sd <- sapply(1:26, function(k){sd(beta_T[,k])})
